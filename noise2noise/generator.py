...
class ValGenerator(Sequence):
    def __init__(self, data_set_path, source_noise_model, target_noise_model):
        ...
        self.x = np.zeros((self.num, self.data_size[0]-4, self.data_size[1]+2, 1), dtype=np.float64)
        self.y = np.zeros((self.num, self.data_size[0]-4, self.data_size[1]+2, 1), dtype=np.float64)

        for sample_id in range(self.num):
            self.x[sample_id] = np.pad(self.source_noise_model(self.data[sample_id]), [(0,0),(1,1),(0,0)], 'constant', constant_values=0)[2:-2]
            self.y[sample_id] = np.pad(self.target_noise_model(self.data[sample_id]), [(0,0),(1,1),(0,0)], 'constant', constant_values=0)[2:-2]
