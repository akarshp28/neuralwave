...
def process_sample(data_path, dest_path, min_, max_, means, pca):
    data = read_array(data_path)
    data -= means
    for i in range(cols):
        data[:, i] = (data[:, i] - min_[i])/(max_[i] - min_[i])
        data[:, i] = smooth(data[:, i], 91)
    data = pca.transform(data.reshape(1, -1)).reshape(rows, cols)
...
scalers_path = "/scratch/kjakkala/neuralwave/data/scalers_pca"
rows = 8000
cols = 540
...
if (rank == 0):
    ...
    mins = []
    maxs = []
    pca = IncrementalPCA(n_components=0.95, batch_size=size)
...
for index in range(num_sl):
    ...
    if (rank == 0):
        train_array -= means
        for i in range(cols):
            train_array[:, i] = (train_array[:, i] - mins[i])/(maxs[i] - mins[i])
            train_array[:, i] = smooth(train_array[:, i], 91)
        pca.fit(train_array.reshape(train_array.shape[0], -1))
        sys.stdout.write("\r{}/{}".format(index+1, num_sl))
        sys.stdout.flush()
...
################################################################################
    ...
    dict = {"means":means, "min":mins, "max":maxs, "pca":pca}
    ...
    data_tmp = [[X_train[i], os.path.join(dest_path, "train"), mins, maxs, means, pca] for i in range(len(X_train))]
    data_tmp.extend([X_test[i], os.path.join(dest_path, "test"), mins, maxs, means, pca] for i in range(len(X_test)))
...
for index in range(data_c_len):
    ...
    process_sample(data_tmp[0], data_tmp[1], data_tmp[2], data_tmp[3], data_tmp[4], data_tmp[5])
